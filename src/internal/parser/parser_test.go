...
func TestParseSimpleSelect(t *testing.T) {
	...
	if len(stmt.Columns) != 1 {
		t.Fatalf("expected 1 column (wildcard), got %d", len(stmt.Columns))
	}
	if stmt.Columns[0].Wildcard == nil {
		t.Fatalf("expected wildcard")
	}
	if len(stmt.From) != 1 {
		t.Fatalf("expected 1 table")
	}
	if stmt.From[0].Name != "events" {
		t.Fatalf("expected table 'events', got %s", stmt.From[0].Name)
	}
}
